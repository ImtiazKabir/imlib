...
PRIVATE void __Init_Key__(register struct ImPair *const self,
                          register void *const key) {
  if ((self->key_policy == POLICY_BORROW) ||
      (self->key_policy == POLICY_TRANSFER)) {
    self->key = key;
  } else {
...
PRIVATE void __Init_Val__(register struct ImPair *const self,
                          register void *const value) {
  if ((self->val_policy == POLICY_BORROW) ||
      (self->val_policy == POLICY_TRANSFER)) {
    self->val = value;
  } else {
...
PRIVATE void __Init__(register struct ImPair *const self,
...
PRIVATE void __Deinit_Key__(register struct ImPair *const self) {
  if ((self->key_policy == POLICY_CLONE) ||
      (self->key_policy == POLICY_TRANSFER)) {
    (void)imdel(self->key);
    self->key = NULL;
  }
...
PRIVATE void __Deinit_Val__(register struct ImPair *const self) {
  if ((self->val_policy == POLICY_CLONE) ||
      (self->val_policy == POLICY_TRANSFER)) {
    (void)imdel(self->val);
    self->val = NULL;
  }
...
PRIVATE void __Constructor__(register void *const self,
  ...
  if (ImParams_Match(args, 4u, PARAM_PTR, PARAM_PTR, PARAM_UNSIGNED,
                     PARAM_UNSIGNED) == IM_FALSE) {
    impanic(
        "%s\n",
  ...
  __Init__(self, key, val, __GetPolicyFromUnsigned__(key_policy),
           __GetPolicyFromUnsigned__(val_policy));
}

PRIVATE void __Destructor__(register void *const self) { __Deinit__(self); }

PUBLIC void ImPair_SetKey(register struct ImPair *const self,
                          register void *const key) {
  if (self->key != NULL) {
    __Deinit_Key__(self);
  }
...
PUBLIC void ImPair_SetValue(register struct ImPair *const self,
                            register void *const value) {
  if (self->val != NULL) {
    __Deinit_Val__(self);
  }
...
PUBLIC void *ImPair_KeyView(struct ImPair const *self) { return self->key; }

PUBLIC void *ImPair_ValueView(struct ImPair const *self) { return self->val; }

PRIVATE char *__ToStr__(void const *const _self) {
  register struct ImPair const *const self = _self;
  register struct ImStr *const str = imnew(ImStr, 0u);
  register char *ret = NULL;
  register char *const key_str = imtostr(self->key);
  register char *const val_str = imtostr(self->val);

  ImStr_AppendFmt(str, "<%s, %s>", key_str, val_str);
  ret = imtostr(str);
