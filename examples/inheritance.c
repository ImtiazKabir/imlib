...
PRIVATE void StringDemo(void) {
  ...
  (void)imprintf("I can print you %obj %d times\n", str1, 4);
  imlog2(LOG_TRACE, "I can print you %obj %d times\n", str1, 4);
  (void)imdel(str1);
}
...
PRIVATE void ImWrapDemo(void) {
  struct ImFloat *wrap = ImFloat_Construct(12.3f);
  (void)imputobj(wrap, stdout);
  ImFloat_Destroy(wrap);
}
